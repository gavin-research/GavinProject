...
async function main() {
  try {
    ...
    //Si es SCAccess despliega SCAuthAccess
    if (mainSC == "SCAccess") {
      var { abi, bytecode } = JSON.parse(
        fs.readFileSync(
          `./contracts/${contractDir}/build/contracts/SCAuthAccess.json`,
        ),
      );
      const abiAASC = abi;
      const bytecodeAASC = bytecode;

      const aaAddr = await contractDeploy(abiAASC, bytecodeAASC);
      console.log("SCAuthAccess deployed: ", aaAddr);

      const contract = new web3.eth.Contract(abiAASC, aaAddr);
      const registerAA_abi = contract.methods
        .updateSCAccess(mainAddr)
        .encodeABI();
      await callMethod(registerAA_abi, contract);
    }
