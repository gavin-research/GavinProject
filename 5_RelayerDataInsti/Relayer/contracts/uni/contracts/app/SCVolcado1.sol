...
//noivern
contract SCVolcado1 is IIBCModule {
    ...
    function addIssuer(address issuerAddy, string memory issuerName) external {
        ...
        string memory codeI0xI_IssuerName = string.concat("I0xI", issuerName);
        sendTransfer(
            codeI0xI_IssuerName,
            issuerAddy,
            sourcePort,
    ...
    function addCertificate(
        ...
        address addressholder = address(bytes20(bytes(_holder)));
        holders[_certificate] = addressholder;
        certificate[_code] = _certificate;
        emit AddCertificate(_certificate, addressholder);
        //msg.sender el BAF del issuer
        //string memory mensaje = string(abi.encode(_certificate, _code, msg.sender));

        string memory sep_holder = string.concat("-", _holder); //-holder
        string memory sep_code = string.concat("|", _code); //|code
        string memory otromensaje = string.concat(_certificate, sep_code); //cert|code
        string memory mensaje = string.concat(otromensaje, sep_holder); //cert|code-holder
