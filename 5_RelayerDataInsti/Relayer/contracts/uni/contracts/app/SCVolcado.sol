...
contract SCVolcado is IIBCModule {
    ...
    event PreSendTransfer(string message, address holder);
    event EncodingDebug(bytes encodedData);
    ...
    function addCertificate(
        ...
        // string memory mensaje = string(
        //     abi.encode(_certificate, _code, msg.sender)
        // );

        // Debug the encoding
        bytes memory encoded = abi.encode(_certificate, _code, msg.sender);
        emit EncodingDebug(encoded);

        string memory mensaje = string(encoded);
        emit PreSendTransfer(mensaje, _holder);
