<script>
	...
	import Access from '../components/modals/access.svelte';
...
<AppShell>
	<svelte:fragment slot="header">
		...
		<AppBar background="dark:variant-filled-secondary variant-filled-primary">
			<svelte:fragment slot="lead">
				...
				<strong class="text-xl uppercase">GAVIN Portal</strong>
			</svelte:fragment>
			<a href="/" class="btn bg-initial" data-sveltekit-preload-data="hover"
				>{$t('common.nav_cert')}</a
			>
			<a href="/profile" class="btn bg-initial" data-sveltekit-preload-data="hover"
				>{$t('common.nav_profile')}</a
			>
	...
	<svelte:fragment slot="footer">
		...
		<section
			class="grid grid-cols-3 gap-2 p-4 dark:variant-filled-secondary variant-filled-primary"
			style="justify-items: center"
		>
